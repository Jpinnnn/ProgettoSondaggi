...
routerEmail.post('/postEmailList', async (req, res) => {
    try {
        ...
        // if (!validator.isEmail(nuovaEmail.email)) {
            console.log("PRIMA DELLA MODIFICA: ", nuovaEmail.email)

            check(nuovaEmail.email).isEmail().trim().escape().normalizeEmail()

            //rende tutto lowercase
            // nuovaEmail.email = validator.normalizeEmail({ all_lowercase: true })
            //rimuovo gli spazi bianchi
            // nuovaEmail.email = validator.escape(nuovaEmail.email)

            // nuovaEmail.email = validator.normalizeEmail(nuovaEmail.email)
            console.log("DOPO LA MODIFICA: ", nuovaEmail.email)
        // }
